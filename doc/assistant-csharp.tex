...
No caso de aplicações do tipo servidor, que recebam chamadas, pode ser necessário o uso de mais uma classe: "tecgraf.openbus.ConnectionContext". 
...
Através dela, será possível obter a identidade do cliente ou cadeia de clientes de uma chamada. Para obter a instância \emph{singleton} tanto do \orb como da classe "ConnectionContext",
devemos utilizar a classe estática "tecgraf.openbus.ORBInitializer". Essa classe é responsável por instanciar e configurar o \orb e o "ConnectionContext". No futuro, caso o IIOP.NET suporte 
...
\begin{samplelisting}[language=CSharp]{autent}{Autenticação em um Barramento}
string host = args[0];
...
\begin{samplelisting}[language=CSharp]{component}{Exemplo de criação de um componente \scs}
...
// Cria a faceta Hello para o componente. A classe HelloImpl deve ser implementada de acordo com a especificação do serviço Hello.
...
\begin{samplelisting}[language=CSharp]{registering}{Registro de uma Oferta de Serviço}
...
\begin{samplelisting}[language=CSharp]{hello}{Implementação da faceta Hello}
...
\begin{samplelisting}[language=CSharp]{processExit}{Exemplo de um \emph{ProcessExit}}
...
\begin{samplelisting}[language=CSharp]{hookinstall}{Instalação de um \emph{EventHandler}}
...
\begin{samplelisting}[language=CSharp]{lifetime}{Implementação da faceta Hello sobrescrevendo o método InitializeLifetimeService}
  public class HelloImpl : MarshalByRefObject, Hello {
    ...
...
\begin{samplelisting}[language=CSharp]{clientauth}{Autenticação da Aplicação Cliente}
string host = args[0];
ushort port = Convert.ToUInt16(args[1]);
string entity = args[2];
...
\begin{samplelisting}[language=CSharp]{clientsearch}{Busca por Ofertas de Serviço}
...
que o serviço não conseguiu se comunicar com o outro ponto;
"CORBA::COMM\_FAILURE" significa a mesma coisa, mas a conexão não
conseguiu ser iniciada por algum motivo; "CORBA::OBJECT\_NOT\_EXIST" 
...
"CORBA::SystemException". Um exemplo do tratamento de erros adequado pode ser visto no Código~\ref{lst:clientfacet}.

\begin{samplelisting}[language=CSharp]{clientfacet}{Acesso à Faceta Desejada}
...
\begin{samplelisting}[language=CSharp]{logout}{Finalizando o \assistant}
assistant.Shutdown();
\end{samplelisting}
...
Os métodos relativos a cadeias ficam na classe "ConnectionContext", como mencionado anteriormente. São eles:
...
Como a implementação da classe "ConnectionContext" em C\# é um \emph{singleton}, podemos acessar sua instância facilmente,
e fazer a chamada a "CallerChain" em "sayHello". Um exemplo pode ser visto no Código~\ref{lst:chain}

\begin{samplelisting}[language=CSharp]{chain}{Acessando a cadeia relativa a uma chamada}
