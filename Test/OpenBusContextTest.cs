  ...
  public class OpenBusTest {
    ...
    public static void MyClassInitialize(TestContext testContext) {
      ...
      if (_useSSL) {
        ...
        _busIOR2 = iors2[0];
        _busRef2 = (IComponent)RemotingServices.Connect(typeof(IComponent), _busIOR2);
      }
      else {
    ...
    public void EncodeAndDecodeLegacyChain() {
      lock (Lock) {
        ...
        CallerChainImpl decodedChain = (CallerChainImpl) _context.DecodeChain(encodeChain);
    ...
    public void MultiplexingMultipleClientsAndServers() {
      lock (Lock) {
        ...
        _context.OnCallDispatch =
          (context, busid, loginId, uri, operation) => busid.Equals(conn3.BusId) ? conn3 : conn4;
        try {
          // registra serviço 1
          _context.SetCurrentConnection(conn3);
          ComponentContext component = BuildTestCallerChainInspectorComponent();
    ...
    /// <summary>
    ///   Teste de multiplexação com dois servidores e dois clientes para cada servidor, utilizando barramentos diferentes e múltiplas threads.
    /// </summary>
    [TestMethod]
    public void MultiplexingMultipleClientsAndServersMultiThreaded() {
      lock (Lock) {
        ...
        _context.OnCallDispatch =
          (context, busid, loginId, uri, operation) => busid.Equals(conn5.BusId) ? conn5 : conn6;
        try {
          // registra serviço 1
