namespace tecgraf.openbus {
  ...
  /// 
  /// Método a ser chamado quando uma chamada proveniente de um barramento é 
  /// recebida. Esse método é chamado para determinar a conexão a ser utilizada 
  /// na validação de cada chamada recebida. Se a conexão informada não 
  /// estiver conectada ao mesmo barramento indicado pelo parâmetro 'busid', a 
  /// chamada provavelmente será recusada com um 
  /// CORBA::NO_PERMISSION{InvalidLogin} pelo fato do login provavelmente não
  ...
  public delegate Connection CallDispatchCallback(
    OpenBusContext context, string busid, string loginId, string uri,
    string operation);
}